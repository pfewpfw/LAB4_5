...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        float[] dashPattern = {
                4.0f, 4.0f, 8.0f, 4.0f, 4.0f, 4.0f, 16.0f, 4.0f, 8.0f, 4.0f, 4.0f, 4.0f
        };
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (double[] point : graphicsData) {
            canvas.setColor(isAscending(point) ? Color.GREEN : Color.BLACK);

            int centerX = (int) xyToPoint(point[0], point[1]).getX();
            int centerY = (int) xyToPoint(point[0], point[1]).getY();

            int rayLength = 9;
            int curveDepth = 4;

            // Верхний луч
            canvas.drawLine(centerX - curveDepth, centerY - rayLength/2, centerX, centerY - rayLength);
            canvas.drawLine(centerX + curveDepth, centerY - rayLength/2, centerX, centerY - rayLength);

            // Правый луч
            canvas.drawLine(centerX + rayLength/2, centerY - curveDepth, centerX + rayLength, centerY);
            canvas.drawLine(centerX + rayLength/2, centerY + curveDepth, centerX + rayLength, centerY);
    ...
    private boolean isAscending(double[] point) {
        int wholePartY = (int) point[1];
        return isEven(wholePartY); // Проверяем, является ли целая часть четным числом
    }
